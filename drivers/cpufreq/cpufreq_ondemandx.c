...
static int ondemandx_powersave_bias_setspeed(struct cpufreq_policy *policy,
 ...
 * rate is 10 ms because the user needs immediate reaction from ondemandx
                                    ...
                                    const char *buf, size_t count)
{
	...
	if (!bypass) {
		...
		for_each_online_cpu(cpu) {
			...
			if (dbs_info->cur_policy) {
				/* cpu using ondemandx, cancel dbs timer */
				dbs_timer_exit(dbs_info);
				...
				mutex_lock(&dbs_info->timer_mutex);
				ondemandx_powersave_bias_setspeed(
                                                 dbs_info->cur_policy,
                                                 NULL,
                                                 input);
				mutex_unlock(&dbs_info->timer_mutex);
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	policy = this_dbs_info->cur_policy;
	if (!policy) {
		/* CPU not using ondemandx governor */
...
static int dbs_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (policy->cur < src_freq) {
			/* cancel the next ondemandx sample */
			...
			/* reschedule the next ondemandx sample */
                                ...
                                unsigned int event)
{
	...
	switch (event) {
        case CPUFREQ_GOV_START:
            ...
            if (!ondemandx_powersave_bias_setspeed(
